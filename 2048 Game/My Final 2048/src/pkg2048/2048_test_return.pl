...
play2(FinalRes) :-
	...
	Board0 = [X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12,X13, X14, X15, X16],
	Depth = 3,
	% mabd2yen kda zaawwd al stack bta3k ll2a5er
	set_prolog_stack(global,limit(10*(10**9))),set_prolog_stack(local,limit(10*(10**9))),
	append(Board0,[],TempRes),
	generate(Board0,Board1),generate(Board1,Board2),
		loop(Board2, Depth,TempRes,FinalRes).
...
generate(B1,B2):-
	...
	put2(B1,N,0,B2).
%==========================================================================================
%check if all are zeros then generate ya3m ay 7aga msh kdaaa
loop(B,Depth,ResOld,ResNew):-
	append(ResOld,[],ResNew),
	max_list(B, 2048),nl,writeln('FINISHEEEEEEeD'),!,true.
loop(B,Depth,ResOld,ResNew):-
	append(ResOld,[],ResNew), %problem is that can not be all empty
...
loop(B, Depth,ResOld,ResNew) :-
	...
	append(ResOld,NewBoard,ResNew1), %get values from all before and add the new one
	%then the whole result till now is ResNew1 and will have some additionals will be in "ResNew"
	...
	loop(NewBoard, Depth,ResNew1,ResNew),!.
